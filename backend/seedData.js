...
const seedData = async () => {
  ...
  await User.create({
    name: 'Customer',
    email: 'customer@signifyahon.com',
  ...
  const products = productData.map((p, i) => {
    ...
    const saleP = p.salePrice || (Math.random() > 0.3 ? basePrice - Math.floor(Math.random() * 2000) : undefined);
    return {
      name: p.name,
      ...
      fabric: p.fabric,
      pieces: p.pieces,
      productType: `Embroidered ${p.fabric}`,
  ...
  await Setting.insertMany([
    ...
    { key: 'contactInfo', value: { phone: '+92 300 1234567', email: 'info@signifyahon.com', address: 'Lahore, Pakistan' } },
  ]);

  console.log('Sample data created:');
  console.log('  Admin: admin@signifyahon.com / admin123');
  console.log('  Customer: customer@signifyahon.com / customer123');
};
