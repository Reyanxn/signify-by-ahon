...
router.get('/guest/:guestId', async (req, res) => {
  try {
    const cart = await getCart(null, req.params.guestId);
    res.json(cart || { items: [], total: 0 });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.post('/add', async (req, res) => {
  try {
    ...
    const existingItem = cart.items.find(
      i => i.product.toString() === productId && i.size === (size || '') && i.isStitched === (isStitched || false)
    );
    if (existingItem) {
      existingItem.quantity += quantity || 1;
    } else {
      cart.items.push({ product: productId, size, isStitched, quantity: quantity || 1, price });
    }
    await cart.save();
